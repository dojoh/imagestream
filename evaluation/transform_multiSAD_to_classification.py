...
if __name__ == "__main__":
    # distance file!
    output_file = "/home/o340n/projects/2023-konstanz/data/2023.08.17-community1-temperature-experiment/trainings_data_subset_with_alloutliers_64/allC/results_test/TE64_allC_mSAD_w_alloutliers_enet_v2s_230823_091245_output_distance.npy"
    ...
    threshold = 0.4  # determined by ROC curve if trained with outliers
    # threshold = 0.1  # determined by ROC curve if trained without outliers

    name = Path(output_file).stem

    output = np.load(output_file)

    classification = np.argmin(output, axis=1)
    classification[np.min(output, axis=1) > threshold] = -1  # outliers
    ...
    print(
        tabulate(
            [
                ["class names", "outliers"] + classes[1:],
