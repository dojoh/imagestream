...
def write_images(images, filename, channels, grid_size=[1, 10], image_size=64):
    hues = ['white',
            ...
            'red',
            'deeppink']
    out = PIL.Image.new('RGB', (image_size * grid_size[1], image_size * grid_size[0] * channels.__len__()),
                        color='black')
    minclips = [-2, -2, 0, 0]
    maxclips = [2, 2, 2, 2]
    for i_image, image in enumerate(images):
        for idx, i_channel in enumerate(channels):
            ...
            tmp = PIL.Image.fromarray(
                ((np.clip((image[idx, :, :] - minclip) / (maxclip - minclip), 0, 1)) * 255).astype(np.uint8))
            tmp = ImageOps.colorize(tmp, black="black", white=hues[i_channel])
            out.paste(tmp, (i_image * image.shape[2], idx * image.shape[1]))
            out.paste(tmp,
                      (column * image.shape[2], idx * image.shape[1] + row * image.shape[1] * channels.__len__()))
...
if __name__ == "__main__":
    ...
    path = Path(data_path)
    output_path = os.path.join(path.parent, "results", pattern)
    ...
    for file in natsorted(Path(data_path).rglob('*' + pattern + '*')):
        ...
        #     ids = sorted(random.sample(list(np.where(classification_data == i_class)[0]),
        #                                min(reduce(mul, images_per_page, 1),
        #                                    np.where(classification_data == i_class)[0].__len__())))
        #     write_images([image_data[i] for i in ids],
        #                  os.path.join(output_path, classes[i_class], file.parent.name + ".png"), vis_channels,
