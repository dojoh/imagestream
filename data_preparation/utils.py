...
def cut_pad_image(image, center, max_size, bg=None):
    ...
    if bg == None:
        ...
        image = pad(
            torch.FloatTensor(image), (left, top, right, bottom), bg, "constant"
        )
...
def clean_normalize(data, var):
    for i_data in tqdm(
        ...
        for i_channel in range(data[i_data].shape[2]):
            data[i_data][:, :, i_channel] = (
                data[i_data][:, :, i_channel] / var[i_channel]
            )
    return data
...
def load_cif(filename, channels=None, outliers=None, inliers=None):
